...
class BPR(nn.Module):
    ...
    def getemb(self,pos_edges,filters=None):
        #print("进入forward函数")
        #print(type(pos_edges))
        #print(pos_edges)
        pos_head_embeds = self.embed_user(pos_edges[:, 0])
        #print(pos_head_embeds)
        if filters is not None:
            constant = len(filters) - filters.count(None)
            if constant != 0:
                pos_head_embeds = apply_filters_gcmc(pos_head_embeds, filters)
        return pos_head_embeds

    def encode(self, nodes, filters=None):
        embs = self.embed_user(nodes)
        if filters is not None:
            constant = len(filters) - filters.count(None)
            if constant != 0:
