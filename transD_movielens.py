...
def lr_scheduler(optimizer, decay_lr, num_epochs):
    if decay_lr in ('ms1', 'ms2', 'ms3'):
        ...
        lr_milestones = [2 ** x for x in range(10-decay_lr, 10) if 2 ** x < num_epochs]
...
def apply_filters_nce(args,p_lhs_emb,p_rhs_emb,nce_lhs_emb,nce_rhs_emb,\
        rel_emb,p_batch_var,nce_batch,d_outs,masked_filter_set):
    ''' Doesnt Have Masked Filters yet '''
    filter_l_emb, filter_r_emb = 0,0
...
def train_gcmc(data_loader,counter,args,modelD,optimizerD,\
        fairD_set, optimizer_fairD_set, filter_set):
    '''
    train(train_loader, epoch, args, train_hash, modelD, optimizerD, \
         fairD_set, optimizer_fairD_set, filter_set, experiment)
    counter是用来作为输出的时候进行计数
    train_hash没有用到
    modelD是GCMC的模型，将来要替换成BPR模型
    ...
    for idx, p_batch in data_itr:
        ...
        if args.use_cuda:
            p_batch = p_batch.cuda()
        # 封装tensor，整合了反向传播的部分实现之后才可以用backward函数
        p_batch_var = Variable(p_batch)
        ''' Number of Active Discriminators '''
        constant = len(masked_fairD_set) - masked_fairD_set.count(None)
        ''' Update GCMC Model '''
        if constant != 0:
            user=p_batch_var[:,0]
            item_i=p_batch_var[:,1]
            item_j=p_batch_var[:,2]
            prediction_i, prediction_j = modelD(user, item_i, item_j)
            loss= -(prediction_i - prediction_j).sigmoid().log().sum()
            lhs_emb = modelD.getemb(p_batch_var, masked_filter_set)
            ...
            if not args.use_cross_entropy:
                ...
                fair_penalty = -1 * l_penalty
            optimizerD.zero_grad()
            full_loss = loss + args.gamma * fair_penalty
            full_loss.backward()
            optimizerD.step()
    ...
    if args.do_log:
        if not args.freeze_transD:
            print("Task Loss")
            print(float(full_loss))
        if fairD_set[0] is not None:
            print("Fair Gender Disc Loss")
            print(float(fairD_gender_loss))
        if fairD_set[1] is not None:
            print("Fair Occupation Disc Loss")
            float(fairD_occupation_loss)
        if fairD_set[2] is not None:
            print("Fair Age Disc Loss")
            print(float(fairD_age_loss))
        if fairD_set[3] is not None:
            print("Fair Random Disc Loss")
            print(float(fairD_age_loss))
def train_bpr(data_loader,counter,args,train_hash,modelD,optimizerD,\
        fairD_set, optimizer_fairD_set, filter_set, experiment):
    '''
        train(train_loader, epoch, args, train_hash, modelD, optimizerD, \
             fairD_set, optimizer_fairD_set, filter_set, experiment)
...
def train(data_loader, counter, args, modelD, optimizerD,\
    ...
    if args.use_gcmc:
        train_gcmc(data_loader,counter,args,modelD,optimizerD,\
                fairD_set, optimizer_fairD_set, filter_set)
    else:
        train_nce(data_loader,counter,args,modelD,optimizerD,\
                fairD_set, optimizer_fairD_set, filter_set)